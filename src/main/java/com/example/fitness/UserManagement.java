	 ...
	 private static final Logger logger = Logger.getLogger(UserManagement.class.getName());
	    ...
	    private static void viewAllUsers() {
	        try (BufferedReader reader = new BufferedReader(new FileReader(Main.USERS_FILE))) {
	            ...
	            logger.log(Level.INFO, "\u001B[34mUsername\tUser Type\tGender\tAge\tAccount Status\u001B[0m");
	            ...
	            while ((line = reader.readLine()) != null) {
	                ...
	                if (userDetails.length >= 6) {
	                    ...
	                    logger.log(Level.INFO, "\u001B[32m{0}\t{1}\t{2}\t{3}\t{4}\u001B[0m",
	                            new Object[]{username, userType, gender, age, accountStatus});
	                } else {
	    ...
	    private static void approveUser() {
	            ...
	            while ((line = reader.readLine()) != null) {
	                ...
	                if (userDetails[0].equals(usernameToApprove)) {
	                    if ("pending".equalsIgnoreCase(userDetails[5].trim())) {
	                        ...
	                        logger.log(Level.INFO, "\u001B[32mUser '{0}' has been approved successfully.\u001B[0m", usernameToApprove);
	                        userFound = true;
	                    } else {
	                        logger.log(Level.WARNING, "\u001B[31mUser '{0}' is not in a pending state.\u001B[0m", usernameToApprove);
	                        userFound = true;
	                    }
	                }
	        ...
	        if (!userFound) {
	            logger.log(Level.WARNING, "\u001B[31mUser '{0}' not found in the system.\u001B[0m", usernameToApprove);
	        }
	     ...
	     private static void denyUser() {
	            ...
	            while ((line = reader.readLine()) != null) {
	                ...
	                if (userDetails[0].equals(usernameToDeny)) {
	                    if ("pending".equalsIgnoreCase(userDetails[5].trim()) || "accepted".equalsIgnoreCase(userDetails[5].trim())) {
	                        ...
	                        logger.log(Level.INFO, "\u001B[32mUser '{0}' has been denied successfully.\u001B[0m", usernameToDeny);
	                        userFound = true;
	                    } else {
	                        logger.log(Level.WARNING, "\u001B[31mUser '{0}' is not in a pending state.\u001B[0m", usernameToDeny);
	                        userFound = true;
	                    }
	                }
	                writer.write(String.join(",", userDetails));
	        ...
	        if (!userFound) {
	            logger.log(Level.WARNING, "\u001B[31mUser '{0}' not found in the system.\u001B[0m", usernameToDeny);
	        }
